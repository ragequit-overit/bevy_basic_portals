...
pub fn update_portal_cameras(
    ...
    for (portal_parts_entity, portal_parts) in portal_parts_query.iter() {
        ...
        // Main Camera
        let (main_camera_global_transform, main_camera, main_camera_render_target) =
...
fn resize_image_if_needed(
    ...
    main_camera: &Camera,
    main_camera_render_target: &RenderTarget,
    ...
    let Some(main_camera_viewport_size) = get_viewport_size(main_camera, main_camera_render_target, size_params) else {
...
pub(super) fn get_viewport_size(
    ...
    match main_camera.viewport.as_ref() {
        ...
        None => match main_camera_render_target {
